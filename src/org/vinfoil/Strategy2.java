...
public class Strategy2 {
	...
	public ArrayList <FWL> St2(ArrayList <FWL> S0, float paperwidth, float offset, int NK ){
		...
		for(int i=0;i<S.size();i++){
			...
			if(a.getWidth()<100){
				...
				if(t==true){
					...
					return St2(S,paperwidth,offset,NK);
				}
		...
		//int [] nj=new int [6];
		int v=0;//third constraint, the numebr of lanes is bigger then the number of knives+1
		ArrayList <Integer> nj=new ArrayList <Integer> ();
		if(S.size()>NK+1){
			//System.out.println("sta god");
			k=S.size()-NK-1;//number of lanes that need to be merged
			
			IndexUse ind=new IndexUse ();
			S=ind.WhichLaneToMerge(S, NK+1);
