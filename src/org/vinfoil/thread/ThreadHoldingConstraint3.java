...
public class ThreadHoldingConstraint3 extends Thread{

	@Override
	public void run() {
		// TODO Auto-generated method stub
		this.HS=this.Direction3();
		System.out.println("third thread "+this.HS.size());
	}
	public ThreadHoldingConstraint3(){
		//constructor
		super();
	}
	...
	public List<HoldingResult> Direction3(){
		//ArrayList <org.vinfoil.HoldingResult> templist=new ArrayList <org.vinfoil.HoldingResult>();
		...
		//ArrayList <org.vinfoil.Point> listOfEmbeding=new ArrayList <org.vinfoil.Point>();
		...
		for(int i=1;i<ShiftX;i++)
			for(int j=1;j<ShiftY;j++){
				...
				matrixTemp=zp.Shifting(matrixPadded, i, j, this.n1, this.m, 3);
				...
				if(shift==false){
					...
					listOfEmbeding=over.ScaningForEmbedingWithTwoLanes(this.discreteLane1, matrixTemp, this.ScanStep, this.n1, this.m, this.X1min, this.X1max, this.X2min-i, this.X2max-i, this.Y1min, this.Y1max, this.Y2min+j, this.Y2max+j);
					...
					synchronized (listOfEmbeding){
						...
						case 1:
							...
							temp.setDirection(3);
							temp.setShiftX(-i-this.ScanStep);
